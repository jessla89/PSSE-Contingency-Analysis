...
For the planning horizon, Chapter \ref{chap:sum} will describe the system topologies that will be studied. Chapter ~\ref{chap:nonconv} summarizes the number of studied scenarios for which a (N-1) contingency does not report a converged power flow solution. For each studied topology, the number of studied scenarios for which a branch gets overloaded due to a (N-1) contingency is summarised in Chapter ~\ref{chap:ovrld}. Chapter \ref{chap:upemvol} and Chapter \ref{chap:lowemvol} describes the number of times a bus violates the upper and lower emergency voltage range. The summary of AC contingency calculation for 7 topologies spanning 5 years is given in the Chapter \ref{conclusion}. 
