...
\begin{figure}[H]
\centering
\includegraphics[scale=0.55]{y5t6}
...
For the planning horizon, Chapter \ref{chap:sum} will describe the system topologies that will be studied. Chapter ~\ref{chap:nonconv} would summarise the number of studied scenarios for which a (N-1) contingency does not report a converged power flow solution. For each studied topology, the number of studied scenarios for which a branch gets overloaded due to a (N-1) contingency would be summarised in Chapter ~\ref{chap:ovrld}. Chapter \ref{chap:upemvol} and Chapter \ref{chap:lowemvol} would describe the number of times a bus violates the upper and lower emergency voltage range. The summary of AC contingency calculation for 7 topologies spanning 5 years will be given in the Chapter \ref{conclusion}. 
...
\item There would only be 1 time when the contingency SING OPN LIN 3001-3003(1) would report a nonconvergence, which would be before the transmission upgrade of parallel line 3001-3003(2) would have been added to the system giving an additional path for power flow from the system swing generator.
...
\item For the parallel branches 153-154(1) and 153-154(2), for Year 2, Topology 2, when the contingency single line open 202-203(1) was studied, the sensitivity analysis showed that the power flows through the branches 202-152(1), 152-153(1), 153- 54(1), 153-154(2), and back to 154-203(1) and 154 to 205 to reach the main load center. The branches 153-154(1) and 153-154(2) being low capacity branches, reported a loading greater than 100\%.
...
Studying the buses individually, for the Bus 154, not only is the bus 154 a load bus, the branches connecting to the bus are low capacity branches and it can be seen that as the load increases over the years in the planning horizon, the reactive power support already existing in the bus 154 is not enough to make the bus operate within normal voltage range.

The load bus 205 is the highest loaded Bus in the system, this bus is supplied with power from multiple branches connected to it. There is a reactive power support already existing in the Bus and the voltage of the bus is regulated by the generator connected at the Bus 206 to be 0.98 P.U. Even with all the support, it can be seen that the heavy loading in the bus causes it to operate below emergency rating for some contingency cases. 
...
The highest number of lower voltage limit violation in Area 5 is reported by Bus 3008. The bus 3008 is connected with a load without any reactive support. As the load increases over the years, the reactive power requirement of the load increases, and without any reactive power support to regulate the voltage at the bus results in lower emergency voltage range violation of the Bus 3008. 


\chapter{Conculsion}
\label{conclusion} 
Conducting AC contingency calculation on the hypothetical SAVNW system spanning 5 years to test for multiple generation and load scenarios addressing the uncertainity in generations and demands gave an overview of the next set of system upgrade studies that would be needed to be conducted. 

The AC Contingency Calculation is carried out for the Hypothetical SAVNW system to honor the interarea transfer limits and hence each area is assigned a swing generator, with the power supplied by the non swing generators remaining the same. In this condition, when a contingency on the area swing generator is tested, the system swing generators balances the load flow by generating the additional power. The solution settings considered in this manner gives a worst-case system operating point to start the planning process. As the hypothetical SAVNW system is a highly loaded system, studying the nonoptimal operating point will be beneficial as there is a lot of scope for improvement for the system. 
...
From Chapter ~\ref{chap:nonconv}, for all the topologies and scenarios studied for the system nonconvergence, it was seen that the nonconvergence is mainly related to serving of the LIGHTCO area loads that are connected to the buses 205 and 203. The contingencies UNIT 206(1) and SING OPN LIN 205-206(1) caused outage of generator connected at Bus 206 leading to a nonconvergence of load flow. The generator at 206 is serving the highest load in the system connected at Bus 205 and hence loss of this generator leads to a large mismatch in the load flow calculations. The nonconvergence reported in Area 1 due to outage of lines 151-152(1) and 151-152(2) are reported after the addition of generator in Area 2 at Bus 212. The power generated at the bus 212 is transferred to various points in the system and in Area 2, by flowing through the ties to Area 1 and to Area 2 due to lack of transmission upgrades in Area 2. 

In Chapter ~\ref{chap:ovrld}, the loading violations most reported is for the branch 3001-3003(1). The branch 3001-3003(1) is connected to the system swing generator and gets loaded heavily as it being one of the branches that enables the power transfer from swing generator to various loads in the system. When the non-swing generator in the area goes out of service, overloading is reported in the branches that are connected to area swing buses. The loading violations reported in Area 1 and Area 2 are due to the rerouted power flow to the loads connected to the buses 205 and 154 when a (N-1) contingency occurs in either of the Areas. In terms of overloading in Area 5, it was seen that when a requirement of system swing generators to serve the load in other areas becomes a necessity, the branches in area 5 being low-capacity ones, reports loading violations. The branch reporting the greatest number of overloading violations is the branch 3001-3003(1) followed by the branches 101-151(1), 3001-3003(2), 3003-3005(1), 3003-3005(2) as given in Table \ref{sumovrldv}. 
...
\begin{table}[H]
\caption{Summary of Branch loading violation counts}
\label{sumovrldv}
\centering
\scalebox{0.85}{\begin{tabular}{lr}
\toprule
Branch & Overload Count \\
\midrule
3001-3003(1) & 509 \\
101-151(1) & 420 \\
3001-3003(2) & 374 \\
...
\begin{table}[H]
\caption{Summary of Bus Upper voltage violation counts}
\label{sumupv}
\centering
\scalebox{0.85}{\begin{tabular}{rr}
\toprule
Bus & Upper violation counts \\
\midrule
211 & 153 \\
212 & 151 \\
\bottomrule
...
\begin{table}[H]
\caption{Summary of Bus Lower voltage violation counts}
\label{sumlowv}
\centering
\scalebox{0.85}{\begin{tabular}{rr}
\toprule
...
\hline
&&\\
Case & A tested contingency case for a studied scenario. & The contingency case SING OPN LIN 1 101-151(1) is a contingency created by open line FROM BUS 101 [NUC-A 21.600] TO BUS 151 [NUCPANT 500.00] CKT 1. All the (N-1) contingency cases tested for Year 0 to Year 5 are given in Table \ref{contdesc}. \\[2mm] 
&&\\
\hline
\end{tabular}
\end{table}
